...
public class SimplePatternPlayer {
	Pattern pdata = null;
	...
	public SimplePatternPlayer(Pattern pdata, Player player) {
		this.pdata = pdata;
		this.player = player;
	...
	public void paint() {
		...
		for (int i = 0; i < pdata.links.size(); i++) {
			...
			if ((l.nodes.getFirst().time <= time + pdata.beat)
				...
				if (last != -1) {
					if (last == l.n - 1)
						last--;
					Pattern.Note node = l.nodes.get(last);
					Pattern.Note next = l.nodes.get(last + 1);
					int x = calcX(node.x);
					int y = calcY(node.time);
		...
		for (int i = 0; i < pdata.notes.size(); i++) {
			Pattern.Note note = pdata.notes.get(i);
